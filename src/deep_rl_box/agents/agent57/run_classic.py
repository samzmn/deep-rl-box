...
from deep_rl_box.networks.value import Agent57MlpNet, Agent57NetworkInputs
from deep_rl_box.networks.curiosity import NGURndMlpNet, NguEmbeddingMlpNet
from deep_rl_box.agents.agent57 import agent
...
environment_name = 'CartPole-v1'  # Classic control tasks name like CartPole-v1, LunarLander-v3, MountainCar-v0, Acrobot-v1.
num_actors = 8  # Number of actor processes to run in parallel.
replay_capacity = 20000  # Maximum replay size (in number of unrolls stored). Watch for out of RAM.
min_replay_size = 2000  # Minimum replay size before learning starts (in number of unrolls stored). 6250
clip_grad = True  # Clip gradients, default on.
max_grad_norm = 40.0  # Max gradients norm when do gradients clip.

learning_rate = 0.001  # Learning rate for adam.
int_learning_rate = 0.005  # Intrinsic learning rate for adam, this is for embedding and RND predictor networks.
...
unroll_length = 6  # Sequence of transitions to unroll before add to replay.
burn_in = 3  # Sequence of transitions used to pass RNN before actual learning. The effective length of unrolls will be burn_in + unroll_length, two consecutive unrolls will overlap on burn_in steps.
...
ucb_window_size = 90 # Sliding window size of the UCB algorithm.
ucb_beta = 1.0 # Beta for the UCB algorithm.
ucb_epsilon = 0.5 # Exploration epsilon for the UCB algorithm.
...
num_iterations = 400  # Number of iterations to run.
num_train_steps = int(1e3)  # Number of training steps to run per iteration, per actor.
num_eval_steps = int(1e3)  # Number of evaluation steps to run per iteration.
target_net_update_interval = 10  # The interval (measured in Q network updates) to update target Q networks.
actor_update_interval = 50  # The frequency (measured in actor steps) to update actor local Q network.
...
results_csv_path = './logs/agent57_classic.csv'  # Path for CSV log file.
checkpoint_dir = './checkpoints' + '/Agent57/' + environment_name  # Path for checkpoint directory.
...
def main():
    """Trains Agent57 on classic environment."""
    ...
    network = Agent57MlpNet(state_dim=state_dim, action_dim=action_dim, num_policies=num_policies, units=32)
    optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, epsilon=adam_eps)

    # Create RND target and predictor networks.
    rnd_target_network = NGURndMlpNet(state_dim=state_dim, is_target=True, latent_dim=32)
    rnd_predictor_network = NGURndMlpNet(state_dim=state_dim, is_target=False, latent_dim=32)

    # Create embedding networks.
    embedding_network = NguEmbeddingMlpNet(state_dim=state_dim, action_dim=action_dim, embed_size=8)
    ...
    actor_nets = [clone_network(network) for _ in range(num_actors)]
    for net in actor_nets:
    ...
    # Setup checkpoint.
    checkpoint = TensorFlowCheckpoint(environment_name=environment_name, agent_name='Agent57', save_dir=checkpoint_dir)
