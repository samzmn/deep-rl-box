...
class Actor(types_lib.Agent):
    ...
    def __init__(
        ...
        disable_trainability(self._network)
        disable_trainability(self._rnd_target_network)
        disable_trainability(self._rnd_predictor_network)
        disable_trainability(self._embedding_network)
    ...
    def reset(self) -> None:
        ...
        # reset lstm layer's hidden states to its original zeros
        self._network.reset_states()
    ...
    def _prepare_network_input(self, timestep: types_lib.TimeStep) -> Agent57NetworkInputs:
        ...
        s_t=tf.expand_dims(s_t[None], axis=1)  # [B, T, state_shape]
        a_tm1=tf.expand_dims(last_action, axis=0)  # [B, T]
        ext_r_t=tf.expand_dims(ext_r_t[None], axis=0)  # [B, T, 1]
        int_r_t=tf.expand_dims(int_r_t[None], axis=0)  # [B, T, 1]
        policy_index=tf.expand_dims(policy_index, axis=0)  # [B, T]
        one_hot_beta = tf.one_hot(policy_index, self._num_policies, dtype=tf.float32) # [B, T, n_policies]
        one_hot_a_tm1 = tf.one_hot(a_tm1, self._action_dim, dtype=tf.float32) # [B, T, n_actions]
        return [s_t, one_hot_a_tm1, ext_r_t, int_r_t, one_hot_beta]
...
class Learner(types_lib.Learner):
    ...
    def __init__(
        ...
        self._target_network = tf.keras.models.clone_model(self._network)
        ...
        self._rnd_obs_normalizer = normalizer.TensorFlowRunningMeanStd(shape=(state_dim))
        self._replay = replay
