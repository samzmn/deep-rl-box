...
from deep_rl_box.networks.value import R2d2DqnMlpNet, RnnDqnNetworkInputs
from deep_rl_box.agents.r2d2 import agent
...
flags.DEFINE_integer('num_iterations', 500, 'Number of iterations to run.')
flags.DEFINE_integer('num_train_steps', int(2e4), 'Number of training env steps to run per iteration, per actor.')
...
flags.DEFINE_integer('actor_update_interval', 50, 'The frequency (measured in actor steps) to update actor local Q network.')
...
flags.DEFINE_string('checkpoint_dir', './checkpoints/r2d2', 'Path for checkpoint directory.')


def main(argv):
    """Trains R2D2 agent on classic control tasks."""
    del argv
    logging.info(f'Runs R2D2 agent')
    ...
    optimizer = keras.optimizers.Nadam(learning_rate=FLAGS.learning_rate, epsilon=FLAGS.adam_eps)

    # Test network output.
    obs, _ = eval_env.reset()
    x = RnnDqnNetworkInputs(
        s_t=tf.convert_to_tensor(obs[None, None, ...], dtype=tf.float32),
        a_tm1=tf.zeros([1, 1], dtype=tf.int64),
        r_t=tf.zeros([1, 1], dtype=tf.float32),
    ...
    # Setup checkpoint.
    checkpoint = TensorFlowCheckpoint(environment_name=FLAGS.environment_name, agent_name='R2D2', save_dir=FLAGS.checkpoint_dir)
