...
from deep_rl_box.networks.value import C51DqnMlpNet, C51DqnConvNet
...
flags.DEFINE_integer(
    'num_eval_steps', int(4e2), 'Number of evaluation steps (environment steps or frames) to run per iteration.'
)
...
flags.DEFINE_string('load_checkpoint_file', './checkpoints/c51-dqn', 'Load a specific checkpoint file.')
flags.DEFINE_string(
    'recording_video_dir',
    'recordings',
...
def main(argv):
    ...
    # Test network input and output and build the network
    ...
    checkpoint = TensorFlowCheckpoint(environment_name=FLAGS.environment_name, agent_name='C51-DQN', restore_only=True)
