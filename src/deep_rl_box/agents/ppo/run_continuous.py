...
from deep_rl_box.networks.policy import GaussianActorMlpNet, GaussianCriticMlpNet
...
from deep_rl_box.agents.ppo import agent
...
def main(argv):
    ...
    policy_optimizer = tf.keras.optimizers.Adam(policy_network.parameters(), lr=FLAGS.learning_rate)

    critic_network = GaussianCriticMlpNet(state_dim=state_dim, hidden_size=FLAGS.hidden_size)
    critic_optimizer = tf.keras.optimizers.Adam(critic_network.parameters(), lr=FLAGS.baseline_learning_rate)
    ...
    # Setup checkpoint.
    checkpoint = TensorFlowCheckpoint(environment_name=FLAGS.environment_name, agent_name='PPO', save_dir=FLAGS.checkpoint_dir)
