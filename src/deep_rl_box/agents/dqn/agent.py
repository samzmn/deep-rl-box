...
class Dqn(types_lib.Agent):
    ...
    def act(self, timestep: types_lib.TimeStep) -> types_lib.Action:
        """
        return an action by following the e-greedy policy with respect to Q values
        Args:
    ...
    def _choose_action(self, s_t: tf.Tensor) -> tf.Tensor:
        """
        choosing action by deep network
        """
        q_values = self._online_network(s_t).q_values[0]
        a_t = tf.argmax(q_values, axis=-1)  # optimal action according to the DQN
        return a_t
