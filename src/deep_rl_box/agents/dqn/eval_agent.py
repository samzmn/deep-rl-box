...
flags.DEFINE_string('load_checkpoint_dir', './checkpoints/dqn', 'Load a specific checkpoint file.')
flags.DEFINE_string(
    'recording_video_dir',
    'recordings',
...
def main(argv):
    ...
    checkpoint = TensorFlowCheckpoint(environment_name=FLAGS.environment_name, agent_name='DQN', restore_only=True)
    checkpoint.register_pair(('network', network))

    if FLAGS.load_checkpoint_dir:
        checkpoint.restore(FLAGS.load_checkpoint_dir)
