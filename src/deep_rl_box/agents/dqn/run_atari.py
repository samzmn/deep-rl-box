...
flags.DEFINE_bool('compress_state', False, 'Compress state images when store in experience replay.')
...
flags.DEFINE_integer(
    'num_train_steps', int(5e4), 'Number of training steps (environment steps or frames) to run per iteration.'
)
flags.DEFINE_integer(
...
def main(argv):
    ...
    assert obs.shape == (FLAGS.environment_height, FLAGS.environment_width, FLAGS.environment_frame_stack)
    ...
    # Setup checkpoint.
    checkpoint = TensorFlowCheckpoint(environment_name=FLAGS.environment_name, agent_name='DQN', save_dir=FLAGS.checkpoint_dir)
