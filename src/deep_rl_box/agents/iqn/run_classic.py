...
from deep_rl_box.networks.value import IqnMlpNet
from deep_rl_box.agents.iqn import agent
...
flags.DEFINE_integer('num_train_steps', int(2e4), 'Number of training env steps to run per iteration.')
...
flags.DEFINE_string('checkpoint_dir', './checkpoints/IQN', 'Path for checkpoint directory.')


def main(argv):
    """Trains IQN agent on classic control tasks."""
    del argv
    logging.info(f'Runs IQN agent')
    ...
    # Setup checkpoint.
    checkpoint = TensorFlowCheckpoint(environment_name=FLAGS.environment_name, agent_name='IQN', save_dir=FLAGS.checkpoint_dir)
