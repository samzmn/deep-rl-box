...
from deep_rl_box.networks.common import disable_trainability, clone_network
...
class Iqn(types_lib.Agent):
    ...
    def _choose_action(self, timestep: types_lib.TimeStep, epsilon: float) -> int:
        ...
        q_values = self._online_network(s_t, num_taus=self._tau_policy).q_values
    ...
    def _calc_loss(self, transitions: replay_lib.Transition) -> Tuple[tf.Tensor, np.ndarray]:
        ...
        network_output = self._online_network(s_tm1, num_taus=self._tau_policy)
        ...
        q_t_selector = self._online_network(s_t, num_taus=self._tau_policy).q_dist  # [batch_size, action_dim, num_taus]
        target_dist_q_t = self._target_network(s_t, num_taus=self._tau_policy).q_dist  # [batch_size, action_dim, num_taus]
