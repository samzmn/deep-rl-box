...
from deep_rl_box.networks.policy import ActorConvNet
from deep_rl_box.agents.sac import agent
...
def main(argv):
    ...
    q1_optimizer = tf.keras.optimizers.Nadam(learning_rate=FLAGS.q_learning_rate)

    q2_network = DqnConvNet(state_dim=state_dim, action_dim=action_dim)
    q2_optimizer = tf.keras.optimizers.Nadam(learning_rate=FLAGS.q_learning_rate)
    ...
    actors = [
        agent.Actor(
            ...
            policy_network=policy_network.__class__.from_config(policy_network.get_config()),
            action_dim=action_dim,
            min_replay_size=FLAGS.min_replay_size,
    ...
    # Setup checkpoint.
    checkpoint = TensorFlowCheckpoint(environment_name=FLAGS.environment_name, agent_name='SAC', save_dir=FLAGS.checkpoint_dir)
