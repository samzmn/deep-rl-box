...
from deep_rl_box.networks.value import NguDqnMlpNet, NguNetworkInputs
...
min_replay_size = 200  # Minimum replay size before learning starts (in number of unrolls stored). 6250
...
unroll_length = 40  # Sequence of transitions to unroll before add to replay.
burn_in = 20  # Sequence of transitions used to pass RNN before actual learning. The effective length of unrolls will be burn_in + unroll_length, two consecutive unrolls will overlap on burn_in steps.
...
def main():
    """Trains NGU agent on classic environment."""
    print('Running NGU agent')
    np.random.seed(seed)
    ...
    network = NguDqnMlpNet(state_dim=state_dim, action_dim=action_dim, num_policies=num_policies)
    optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, epsilon=adam_eps)
    # Create RND target and predictor networks.
    rnd_target_network = NGURndMlpNet(state_dim=state_dim, is_target=True)
    rnd_predictor_network = NGURndMlpNet(state_dim=state_dim, is_target=False)
