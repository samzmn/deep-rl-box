...
from deep_rl_box.networks.value import QRDqnMlpNet, QRDqnConvNet
...
flags.DEFINE_bool('use_tensorboard', False, 'Use Tensorboard to monitor statistics, default off.')
flags.DEFINE_string('load_checkpoint_file', './checkpoints/qr-dqn', 'Load a specific checkpoint file.')
flags.DEFINE_string(
    'recording_video_dir',
    'recordings',
...
def main(argv):
    ...
    checkpoint = TensorFlowCheckpoint(environment_name=FLAGS.environment_name, agent_name='QR-DQN', restore_only=True)
