...
from deep_rl_box.networks.value import QRDqnMlpNet
from deep_rl_box.agents.qr_dqn import agent
...
flags.DEFINE_integer('replay_capacity', 10_000, 'Maximum replay size.')
...
flags.DEFINE_float('exploration_epsilon_decay_step', 10_000, 'Total steps to decay value of the exploration rate.')
...
flags.DEFINE_integer('num_train_steps', int(2e2), 'Number of training env steps to run per iteration.')
...
flags.DEFINE_string('checkpoint_dir', './checkpoints/qr-dqn', 'Path for checkpoint directory.')


def main(argv):
    """Trains QR-DQN agent on classic control tasks."""
    del argv
    logging.info(f'Runs QR-DQN agent')
    ...
    quantiles = (tf.range(0, num_quantiles, dtype=tf.float32) + 0.5) / float(num_quantiles)
    ...
    # Setup checkpoint.
    checkpoint = TensorFlowCheckpoint(environment_name=FLAGS.environment_name, agent_name='QR-DQN', save_dir=FLAGS.checkpoint_dir)
