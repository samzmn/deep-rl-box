"""
From the paper "Dueling Network Architectures for Deep Reinforcement Learning" https://arxiv.org/abs/1511.06581.
"""
...
from deep_rl_box.networks.value import DuelingDqnMlpNet
from deep_rl_box.agents.dueling_dqn import agent
...
flags.DEFINE_float('eval_exploration_epsilon', 0.0, 'Fixed exploration rate in e-greedy policy for evaluation.')
...
flags.DEFINE_string('results_csv_path', './logs/dueling_dqn_classic_results.csv', 'Path for CSV log file.')
flags.DEFINE_string('checkpoint_dir', './checkpoints/dueling-dqn', 'Path for checkpoint directory.')


def main(argv):
    """Trains DQN agent with dueling deep network on classic control tasks."""
    del argv
    logging.info(f'Runs DQN agent with dueling deep network')
    ...
    network = DuelingDqnMlpNet(state_dim=state_dim, action_dim=action_dim)
    optimizer = tf.keras.optimizers.Nadam(learning_rate=FLAGS.learning_rate)
    ...
    # Create DuelingDqn agent instance
    train_agent = agent.DuelingDqn(
        network=network,
        optimizer=optimizer,
    ...
    eval_agent = greedy_actors.EpsilonGreedyActor(
        ...
        random_state=random_state,
        name='Dueling-DQN-greedy',
    )

    # Setup checkpoint.
    checkpoint = TensorFlowCheckpoint(
        environment_name=FLAGS.environment_name, agent_name='Dueling-DQN', save_dir=FLAGS.checkpoint_dir
    )
    ...
    main_loop.run_single_thread_training_iterations(
        ...
        num_eval_steps=0, #FLAGS.num_eval_steps,
