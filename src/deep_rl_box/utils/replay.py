...
class Transition(NamedTuple):
    ...
    s_tm1: Optional[np.ndarray] # state t minus one
    a_tm1: Optional[int] # action t minus one
    r_t: Optional[float] # reward t
    s_t: Optional[np.ndarray] # state t
    done: Optional[bool] # done or truncated
...
class UniformReplay(Generic[ReplayStructure]):
    ...
    def __init__(
        ...
        self._encoder = encoder or default_encoder_decoder
        self._decoder = decoder or default_encoder_decoder
    ...
    def num_added(self) -> int:
        """Total number of samples added to the replay."""
...
class PrioritizedReplay:
    ...
    def add(self, item: ReplayStructure, priority: float) -> None:
        ...
        self._buffer[self._index] = self._encoder(item)
        self._index += 1
        if not self._full and self._index == self._capacity:
            self._full = True

    def get(self, indices: Sequence[int]) -> Iterable[ReplayStructure]:
        """Retrieves transitions by indices."""
        return [self._decoder(self._buffer[i]) for i in indices]
    ...
    def sample(self, size: int) -> Tuple[ReplayStructure, np.ndarray, np.ndarray]:
        ...
        if self._priority_exponent == 0:
            ...
            indices = self._random_state.choice(np.arange(probs.shape[0]), size=size, replace=False, p=probs)
    ...
    def size(self) -> None:
        ...
        return self._capacity if self._full else min(self._index, self._capacity)
    ...
    def importance_sampling_exponent(self):
        ...
        return self._importance_sampling_exponent(self._index)


class EagerPrioritizedReplay:
    def __init__(
        self,
        capacity: int,
        structure: ReplayStructure,
    ...
    def add(self, item: ReplayStructure, priority: float) -> None:
        ...
        if not np.isfinite(priority) or priority < 0.0:
            raise ValueError('priority must be finite and positive.')
        if self._full:
            self._index = np.argmin(self._priorities)
        else:
            self._index += 1
    ...
    def sample(self, size: int) -> Tuple[ReplayStructure, np.ndarray, np.ndarray]:
        ...
        samples = self.get(indices)
        stacked = np_stack_list_of_transitions(samples, self.structure, 0)
        return stacked, indices, weights
    ...
    def update_priorities(self, indices: Sequence[int], priorities: Sequence[float]) -> None:
        ...
        for index, priority in zip(indices, priorities):
            self._priorities[index] = priority
        self._priorities /= np.max(self._priorities)
...
class GradientReplay(Generic[ReplayStructure]):
    ...
    def __init__(self, capacity: int, network: tf.keras.Model, compress: bool) -> None:
        if capacity <= 0:
            raise ValueError(f'Expect capacity to be a positive integer, got {capacity}')
        super().__init__()
        self._capacity = capacity  # batch size
        self._decode = uncompress_array if compress else default_encoder_decoder
    ...
    def add(self, gradients: List[np.ndarray]) -> None:
        """Store extracted gradients extract with [param.numpy() for param in net.trainable_variables]"""
...
class NStepTransitionAccumulator:
    ...
    *   `LAST`: yield all transitions that end at `s_t = s_T` from up to n steps
    ...
    def step(self, timestep_t: types_lib.TimeStep, a_t: int) -> Iterable[Transition]:
        ...
        if timestep_t.done:
            ...
            while self._transitions:
                yield self._build_n_step_transition(self._transitions, self._discount)
    ...
    def _build_n_step_transition(self, transitions: Iterable[Transition], discount: float) -> Transition:
...
def default_encoder_decoder(s):
    return s

def stack_list_of_transitions(transitions, structure, axis=0):
    """
    ...
    stacked = [tf.stack(xs, axis=axis) for xs in transposed]
...
def split_structure(input_, structure, prefix_length: int, axis: int = 0) -> Tuple[ReplayStructure]:
    ...
    if prefix_length == 0:
        ...
        split = [
            np.split(
                xs,
                [prefix_length, xs.shape[axis]],  # for tf.split() [prefix_length, xs.shape[axis] - prefix_length],
                axis=axis,
            )
            for xs in input_
        ]
