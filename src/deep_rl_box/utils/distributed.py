...
def calculate_dist_priorities_from_td_error(td_errors: tf.Tensor, eta: float) -> np.ndarray:
    """Calculate priorities for distributed experience replay, follows Ape-x and R2D2 papers."""

    td_errors = tf.identity(td_errors)
    abs_td_errors = tf.abs(td_errors)

    priorities = eta * tf.reduce_max(abs_td_errors, axis=1) + (1 - eta) * tf.reduce_mean(abs_td_errors, axis=1)
    priorities = tf.clip_by_value(priorities, clip_value_min=0.0001, clip_value_max=1000)  # Avoid NaNs
...
def get_ngu_policy_betas(
    ...
    """Returns list of intrinsic reward scale betas, following the 'Distributed training' in the paper at Appendix.
