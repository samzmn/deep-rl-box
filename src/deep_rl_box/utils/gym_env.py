...
CLASSIC_ENV_NAMES = ['CartPole-v1', 'LunarLander-v3', 'MountainCar-v0', 'Acrobot-v1']
...
class LifeLoss(gym.Wrapper):
    ...
    def step(self, action):
        ...
        self.was_real_terminated = done or truncated
...
def create_atari_environment(
    ...
    Args:
        ...
        channel_first: if True, change observation image from shape [H, W, C] to in the range [C, H, W], this is for PyTorch only, default off.
    ...
    env = gym.make(f'{env_name}NoFrameskip-v4', render_mode=render_mode, obs_type="rgb") # obs_type="rgb" or "grayscale" or "ram"
    env.reset(seed=seed)
...
def play_and_record_video(
    ...
    while True:
        ...
        observation, reward, done, truncated, _ = env.step(a_t)
        t += 1
...
def plot_animation(frames, name=None, save=False, repeat=False, interval=40):
...
def show_one_episode(env, agent, n_max_steps=1000, name=None, save=False, seed=42):
    frames = []
    np.random.seed(seed)
    ...
    for step in range(n_max_steps):
        ...
        if done or truncated:
            break
    env.close()
    return plot_animation(frames, name, save)