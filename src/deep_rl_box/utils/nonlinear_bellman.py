...
def transformed_retrace(
    ...
    Args:
      q_tm1: Q-values at time t-1, this is from the online Q network, shape [B, T, action_dim].
      q_t: Q-values at time t, this is often from the target Q network, shape [B, T, action_dim].
      a_tm1: action index at time t-1, the action the agent took in state s_tm1, shape [B, T].
      a_t: action index at time t, the action the agent took in state s_t, shape [B, T].
      r_t: reward at time t, shape [B, T].
      discount_t: discount at time t, shape [B, T].
      pi_t: target policy probs at time t, shape [B, T, action_dim].
      mu_t: behavior policy probs at time t, shape [B, T, action_dim].
      lambda_: scalar mixing parameter lambda.
      eps: small value to add to mu_t for numerical stability.

    Returns:
      Transformed retrace td error, shape [B, T].
    """
    ...
    c_t = tf.minimum(1.0, pi_a_t / (mu_t + eps)) * lambda_
