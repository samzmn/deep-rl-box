...
def run_one_episode(agent: types_lib.Agent, env: gym.Env, max_steps: int, trackers: Iterable[Any], seed: int=None) -> Mapping[Text, float]:
    """
    ...
    for _ in range(max_steps):  # For each step in the current episode.
        ...
        if done or truncated or loss_life:  # Actual end of an episode
            ...
            statistic_sequence.append((env, timestep_t, agent, a_t))
            break
    stats = trackers_lib.generate_statistics(trackers, statistic_sequence)
    return stats, sum(rewards)
...
def run_single_thread_training_iterations(
    ...
    train_env: gym.Env,
    eval_agent: types_lib.Agent =False,
    eval_env: gym.Env = False,
    checkpoint: TensorFlowCheckpoint = None,
    csv_file: str = None,
    ...
    for iteration in range(1, num_iterations + 1):
        # Run training steps.
        train_stats, train_score = run_one_episode(train_agent, train_env, num_train_steps, train_trackers, seed)

        # Saving the network and agent parameters
        if checkpoint and not should_run_evaluator:
            checkpoint.set_iteration(iteration)
            checkpoint.save(train_score)
        ...
        log_std_out = [
            ...
            f'best_train_score: {best_train_score:4.0f}',
            f'train_score: {train_score:4.0f}',
        ]
        log_csv_output = [
            ('iteration', iteration),
            ('best_train_score', best_train_score),
            ('train_score', train_score),
            ('train_step', iteration * num_train_steps),
            ('train_episode_return', train_stats['mean_episode_reward']),
            ('train_num_episodes', train_stats['num_episodes']),
        ...
        if should_run_evaluator is True:
            ...
            eval_stats, eval_score = run_one_episode(eval_agent, eval_env, num_eval_steps, eval_trackers, seed)

            # Saving the network and agent parameters
            if checkpoint:
                checkpoint.set_iteration(iteration)
                checkpoint.save(eval_score)
            ...
            eval_std_output = [
                ...
                'eval_episode_reward: {:4.0f}'.format(np.array(eval_stats['mean_episode_reward']).item()),
                f'best_eval_ecore {best_eval_score: 4.0f}',
            ...
            log_std_out.extend(eval_std_output)
            eval_csv_output = [
                ('eval_step', iteration * num_eval_steps),
                ('eval_episode_return', eval_stats['mean_episode_reward']),
                ('eval_num_episodes', eval_stats['num_episodes']),
                ('eval_duration', eval_stats['duration']),
            ]
            log_csv_output.extend(eval_csv_output)

        log_output_str = ' - '.join(log_std_out)
        print(f"\r{log_output_str}", end="")
        writer.write(collections.OrderedDict((n, v) for n, v in log_csv_output))

    print()
    writer.close()
    # extra code – this cell plots the learning curve
    plt.figure(figsize=(10, 5))
    plt.title("Reward Progression Over Episodes", fontsize=16)
    plt.plot(train_scores, linestyle="--", color="r")
    plt.xlabel("Episode", fontsize=14)
    plt.ylabel("Sum of rewards", fontsize=14)
    plt.grid(True)
    plt.savefig(f"{train_agent.agent_name}-{train_env.spec.id}-reward_plot.png", dpi=300, bbox_inches="tight")
    # Verify current backend
    if plt.get_backend() in ["TkAgg", "Qt5Agg", "GTK3Agg"]:
        plt.show()
...
def run_parallel_training_iterations(
    ...
    iteration_count = multiprocessing.Value('i', 0) # signed integer
...
def run_actor(
    ...
    * The actor whoever finished the current iteration first will reset `start_iteration_event` to False,
    ...
    while not stop_event.is_set():
        ...
        # Run training steps.
        train_stats, last_step = run_one_episode(actor, actor_env, num_train_steps, actor_trackers)
        ...
        print(f"iter: {iteration}, role: {actor.agent_name}, episode reward: {train_stats['mean_episode_reward']}")
        # log_output = [
...
def run_learner(
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        if should_run_evaluator is True:
            # Run some evaluation steps.
            eval_stats, last_step = run_one_episode(eval_agent, eval_env, num_eval_steps, eval_trackers)
            ...
            # ]
            #log_queue.put(log_output)
            print(f"Evaluation Episode {iteration} Reward: {eval_stats['mean_episode_reward']}")
...
def run_logger(log_queue: queue.SimpleQueue, csv_file: str):
...
def run_evaluation_iterations(
    ...
    if num_iterations > 0 and num_eval_steps > 0:
        ...
        for iteration in range(1, num_iterations + 1):
            ...
            eval_stats, score = run_one_episode(eval_agent, eval_env, num_eval_steps, test_trackers)
        ...
        print("mean episodes score: ", mean_rewards)
...
def get_tb_log_prefix(env_id: str, agent_name: str, tag: str, suffix: str) -> str:
    ...
    tb_log_prefix += f'-{datetime.datetime.now().strftime("%Y_%m_%d-%H_%M")}'
...
def handle_exit_signal():
    ...
    def shutdown(signal_code, frame):
        del frame
        print(f'Received signal {signal_code}: terminating process...',)
