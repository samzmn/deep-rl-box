...
def n_step_bellman_target(
    ...
    bellman_target = tf.concat(
        [tf.zeros_like(q_t[0:1]), q_t] + [q_t[-1:] / gamma**k for k in range(1, n_steps)], axis=0
    )
    ...
    rewards = tf.concat([r_t] + [tf.zeros_like(r_t[0:1])] * n_steps, axis=0)
    ...
    for _ in range(n_steps):
        ...
        bellman_target = rewards + gamma * (1.0 - tf.cast(done, tf.float32)) * bellman_target[1:]
...
def truncated_generalized_advantage_estimation(
    ...
    base.assert_rank_and_dtype(value_t, 1, tf.float32)
    base.assert_rank_and_dtype(value_tp1, 1, tf.float32)
    base.assert_rank_and_dtype(discount_tp1, 1, tf.float32)

    lambda_ = tf.ones_like(discount_tp1) * lambda_  # If scalar, make into vector.

    delta_t = r_t + discount_tp1 * value_tp1 - value_t

    advantage_t = tf.TensorArray(dtype=tf.float32, size=tf.shape(delta_t)[0])

    gae_t = 0.0
    for i in tf.range(tf.shape(delta_t)[0] - 1, -1, -1):
        gae_t = delta_t[i] + discount_tp1[i] * lambda_[i] * gae_t
        advantage_t = advantage_t.write(i, gae_t)

    return advantage_t.stack()
...
def general_off_policy_returns_from_action_values(
    ...
    base.assert_rank_and_dtype(c_t, 2, tf.float32)
    ...
    exp_q_t = tf.reduce_sum(pi_t * q_t, axis=-1)
...
def general_off_policy_returns_from_q_and_v(
    ...
    base.assert_rank_and_dtype(q_t, 2, tf.float32)
    base.assert_rank_and_dtype(v_t, 2, tf.float32)
    base.assert_rank_and_dtype(r_t, 2, tf.float32)
    base.assert_rank_and_dtype(discount_t, 2, tf.float32)
    base.assert_rank_and_dtype(c_t, 2, tf.float32)
    ...
    return tf.stack(returns, axis=0)
