...
import shutil
import collections
from typing import Any, Text, Iterable, Mapping, Optional, Tuple, Union, List
import numpy as np
...
class EpisodeTracker:
    ...
    def step(self, env, timestep_t, agent, a_t) -> None:
        ...
        if timestep_t.done:
            self._episode_rewards.append(sum(self._current_episode_rewards))
            self._episode_steps.append(self._current_episode_step)
            self._current_episode_rewards = [] # reset
            self._current_episode_step = 0 # reset
    ...
    def get(self) -> Mapping[str, Union[int, float, None]]:
        ...
        `mean_episode_reward` which is the mean return of complete episodes only. If
        ...
        if len(self._episode_rewards) > 0:
            mean_episode_reward = np.array(self._episode_rewards).mean()

            if len(self._episode_visited_rooms) > 0:
                mean_episode_visited_rooms = np.array(self._episode_visited_rooms).mean()
        else:
            mean_episode_reward = sum(self._current_episode_rewards)
...
class TensorboardEpisodeTracker(EpisodeTracker):
    ...
    def step(self, env, timestep_t, agent, a_t) -> None:
        ...
        if timestep_t.done:
            ...
            # tracker per episode
            episode_return = self._episode_rewards[-1]
            episode_step = self._episode_steps[-1]

            # tracker per step
            with self._writer.as_default():
                tf.summary.scalar('performance(env_steps)/num_episodes', self._total_episodes, step=tb_steps)
                tf.summary.scalar('performance(env_steps)/episode_reward', episode_return, step=tb_steps)
                tf.summary.scalar('performance(env_steps)/episode_steps', episode_step, step=tb_steps)

            # For Atari games like MontezumaRevenge and Pitfall
            if isinstance(timestep_t.info, dict) and 'episode_visited_rooms' in timestep_t.info:
                episode_visited_rooms = self._episode_visited_rooms[-1]
                with self._writer.as_default():
                    tf.summary.scalar('performance(env_steps)/episode_visited_rooms', episode_visited_rooms, step=tb_steps)
...
class TensorboardStepRateTracker(StepRateTracker):
    ...
    def step(self, env, timestep_t, agent, a_t) -> None:
        ...
        if timestep_t.done:
            time_stats = self.get()
            with self._writer.as_default():
                tf.summary.scalar('performance(env_steps)/step_rate', time_stats['step_rate'], step=self._total_steps)
                ...
                if stats:
                    for k, v in stats.items():
                        if isinstance(v, (int, float)):
                            with self._writer.as_default():
                                tf.summary.scalar(f'agent_statistics(env_steps)/{k}', v, step=self._total_steps)
            except Exception:
                pass
...
class TensorboardScreenshotTracker:
    ...
    def __init__(self, writer: tf.summary.SummaryWriter, log_interval: int = 100):
        self._total_steps = 0  # keep track total number of steps, does not reset
        self._total_episodes = 0  # keep track total number of episodes, does not reset
...
class TensorboardLearnerStatisticsTracker:
    ...
    def step(self, stats) -> None:
        ...
        if self._total_steps % 100 == 0:
            time_stats = self.get()
            with self._writer.as_default():
                tf.summary.scalar('learner_statistics(learner_steps)/step_rate', time_stats['step_rate'], step=self._total_steps)
            ...
            try:
                if stats:
                    for k, v in stats.items():
                        if isinstance(v, (int, float)):
                            with self._writer.as_default():
                                tf.summary.scalar(f'learner_statistics(learner_steps)/{k}', v, step=self._total_steps)
            except Exception:
                pass
...
def make_default_trackers(log_dir=None, debug_screenshots_interval=0, episode=True, agent=True, step_rate=True):
    """
    ...
    if log_dir:
        ...
        if agent:
            trackers.append(TensorboardAgentStatisticsTracker(writer))
        if step_rate:
        ...
        if episode:
            trackers.append(EpisodeTracker())
        if step_rate:
            trackers.append(StepRateTracker())
...
def generate_statistics(
    ...
    timestep_action_sequence: List[Tuple[Optional[replay_lib.Transition]]],
) -> Mapping[str, Any]:
